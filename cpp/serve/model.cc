...
class ModelImpl : public ModelObj {
 public:
  ...
  NDArray GetLogits(const ObjectRef& hidden_states, int batch_size, int seq_len) final {
    ...
    if (ft_.use_disco) {
      NDArray hidden_states_nd = Downcast<DRef>(hidden_states)->DebugGetFromRemote(0);
      const void* p = hidden_states_nd.get();
      LOG(INFO) << p;
    }
    ObjectRef ret = ft_.get_logits_func_(hidden_states_dref_or_nd, params_);
    if (ft_.use_disco) {
      ObjectRef result = Downcast<DRef>(ret)->DebugGetFromRemote(0);
      LOG(INFO) << result.defined();
      LOG(INFO) << result->GetTypeKey();
      ft_.sess->SyncWorker(0);
      ft_.sess->SyncWorker(1);
    }
    if (trace_enabled_) {
  ...
  ObjectRef BatchSelectLastHidden(const ObjectRef& last_hidden_states,
                                  const std::vector<int>& indices) final {
    ...
    logit_pos_nd.CopyFromBytes(indices.data(), indices.size() * sizeof(int));
    ...
    LOG(INFO) << "BSL";
    if (ft_.use_disco) {
      ObjectRef obj = Downcast<DRef>(last_hidden_states)->DebugGetFromRemote(0);
      LOG(INFO) << obj.defined();
      obj = Downcast<DRef>(logit_pos_dref_or_nd)->DebugGetFromRemote(0);
      LOG(INFO) << obj.defined();
    }
    ...
    ObjectRef hidden_states_dref_or_nd = last_hidden_states;
    // hidden_states: (b, s, h)

    // hidden_states_dref_or_nd = hidden_states.CreateView(
    // {hidden_states->shape[0] * hidden_states->shape[1], hidden_size_}, hidden_states->dtype);
    // ft_.sess->SyncWorker(0);
    // ft_.sess->SyncWorker(1);
  ...
  ObjectRef FuseEmbedHidden(const ObjectRef& embeddings, const ObjectRef& previous_hidden_states,
    ...
    LOG(INFO) << "Bs= " << batch_size << " seq len = " << seq_len;
    ...
    if (ft_.use_disco) {
      ObjectRef obj = Downcast<DRef>(embeddings_dref_or_nd)->DebugGetFromRemote(0);
      LOG(INFO) << obj.defined();
      LOG(INFO) << obj->GetTypeKey();
      obj = Downcast<DRef>(previous_hidden_states)->DebugGetFromRemote(0);
      LOG(INFO) << obj.defined();
      LOG(INFO) << obj->GetTypeKey();
      obj = Downcast<DRef>(params_)->DebugGetFromRemote(0);
      LOG(INFO) << obj.defined();
      LOG(INFO) << obj->GetTypeKey();
      Array<ObjectRef> params = Downcast<Array<ObjectRef>>(obj);
      for (int i = 0; i < params.size(); i++) {
        LOG(INFO) << params[i].defined();
        LOG(INFO) << params[i]->GetTypeKey();
      }

    }
    ObjectRef fused =
        ft_.fuse_embed_hidden_func_(embeddings_dref_or_nd, previous_hidden_states, params_);
    if (trace_enabled_) {
      TVMSynchronize(device_.device_type, device_.device_id, nullptr);
    }
    if (ft_.use_disco) {
      ObjectRef obj = Downcast<DRef>(fused)->DebugGetFromRemote(0);
      LOG(INFO) << obj.defined();
    }
  ...
  ObjectRef BatchPrefillToLastHidden(const ObjectRef& embedding_or_hidden_states,
    ...
    if (ft_.use_disco) {
      ObjectRef obj = Downcast<DRef>(result)->DebugGetFromRemote(0);
      LOG(INFO) << obj.defined();
    }
    ObjectRef hidden_states = ft_.tuple_getitem_func_(result, 0);

    if (ft_.use_disco) {
      ObjectRef obj = Downcast<DRef>(hidden_states)->DebugGetFromRemote(0);
      LOG(INFO) << obj.defined();
    }
    if (trace_enabled_) {
      TVMSynchronize(device_.device_type, device_.device_id, nullptr);
  ...
  ObjectRef BatchVerifyToLastHidden(const ObjectRef& embeddings,
    ...
    if (!ft_.use_disco) {
      ...
      return ft_.nd_view_func_(Downcast<DRef>(hidden_states), out_shape);
    }
  }
  ...
  // virtual void GatherHiddenStates(const ObjectRef& input, const std::vector<int>& indices,
  ...
  ObjectRef GatherHiddenStates(const ObjectRef& input, const std::vector<int>& indices,
                          const ObjectRef& dst) final {
    ShapeTuple out_shape{static_cast<int64_t>(indices.size()), hidden_size_};
    ObjectRef dst_dref_or_nd{nullptr};
    if (ft_.use_disco) {
      dst_dref_or_nd = ft_.nd_view_func_(dst, out_shape);
    } else {
      NDArray dst_nd = Downcast<NDArray>(dst);
      dst_dref_or_nd = dst_nd.CreateView(out_shape, dst_nd->dtype);
    }

    NDArray indices_nd =
        logit_pos_arr_.CreateView({static_cast<int64_t>(indices.size())}, DataType::Int(32));
    indices_nd.CopyFromBytes(indices.data(), indices.size() * sizeof(int));
    ObjectRef indices_dref_or_nd = ft_.CopyToWorker0(indices_nd, "logit_pos", {max_num_sequence_});
    ft_.gather_hidden_states_func_(input, indices_dref_or_nd, dst_dref_or_nd);
    return dst_dref_or_nd;
  }
  ...
  void ScatterHiddenStates(const ObjectRef& input, const std::vector<int>& indices,
    ...
    indices_nd.CopyFromBytes(indices.data(), indices.size() * sizeof(int));
    ObjectRef indices_dref_or_nd = ft_.CopyToWorker0(indices_nd, "logit_pos", {max_num_sequence_});
    ft_.scatter_hidden_states_func_(input, indices_dref_or_nd, dst);
  }

  void GatherDraftProbs(const NDArray& input, const std::vector<int>& indices,
                        const NDArray& dst) final {
    NDArray indices_nd =
        logit_pos_arr_.CreateView({static_cast<int64_t>(indices.size())}, DataType::Int(32));
    indices_nd.CopyFromBytes(indices.data(), indices.size() * sizeof(int));
    bool use_disco = ft_.use_disco;
    ...
    ObjectRef indices_dref_or_nd = ft_.CopyToWorker0(indices_nd, "logit_pos", {max_num_sequence_});
    ft_.use_disco = use_disco;
    ft_.gather_probs_func_(input, indices_dref_or_nd, dst);
  }
  ...
  void ScatterDraftProbs(const NDArray& input, const std::vector<int>& indices,
    ...
    ObjectRef indices_dref_or_nd = ft_.CopyToWorker0(indices_nd, "logit_pos", {max_num_sequence_});
    ft_.use_disco = use_disco;
    ft_.scatter_probs_func_(input, indices_dref_or_nd, dst);
  }
